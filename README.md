...
```toml
[github]
repo = "kunthive-Labs/Margana"
```

Public repos work without a token, but you'll hit GitHub's 60 req/hr unauthenticated limit fast. For private repos or higher rate limits, provide a [personal access token](https://github.com/settings/tokens) via the `MARGA_GITHUB_TOKEN` env var. The token is stored in your OS keyring (macOS Keychain, Linux Secret Service, Windows Credential Manager) — never in `config.toml`. If you paste it into the file as a one-time bootstrap, Marga migrates it to the keyring and strips the plaintext line on next save.
