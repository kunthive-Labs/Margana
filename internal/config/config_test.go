...
func clearConfigEnvVars() {
	...
	os.Unsetenv("MARGA_THEME")
	os.Unsetenv("MARGA_HISTORY_LIMIT")
	os.Unsetenv("MARGA_GITHUB_TOKEN")
	os.Unsetenv("MARGA_GITHUB_REPO")
}

func TestGithubEnvOverrides(t *testing.T) {
	cfg := Default()

	os.Setenv("MARGA_GITHUB_TOKEN", "ghp_envtoken")
	os.Setenv("MARGA_GITHUB_REPO", "octocat/hello-world")
	defer func() {
		os.Unsetenv("MARGA_GITHUB_TOKEN")
		os.Unsetenv("MARGA_GITHUB_REPO")
	}()
	...
	if cfg.Github.Token != "ghp_envtoken" {
		t.Errorf("expected MARGA_GITHUB_TOKEN to set github.token, got '%s'", cfg.Github.Token)
	}
	if cfg.Github.Repo != "octocat/hello-world" {
		t.Errorf("expected MARGA_GITHUB_REPO to set github.repo, got '%s'", cfg.Github.Repo)
	}
}
